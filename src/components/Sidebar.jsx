...
const Sidebar = (props) => {
  ...
  const handleSubmit = async (e) => {
    ...
    let arr = formDetails.file.map((fileObj) => {
      // [promise , promise]
      // let x = covertInString(fileObj)
      // return x
      let formData = new FormData();
      formData.append("file", fileObj);
      formData.append("upload_preset", "socialmedia"); // BlobApp -->upload preset name
      let res1 = axios.post(
        `https://api.cloudinary.com/v1_1/dfoj68y7q/upload`,
        formData
      ); //dsf7eyovf is your cloud name
    ...
    let res = await axios.post("https://pixify-backend-phx3.onrender.com/post/create", finalObj, {
      headers: {
        Authorization: userStore.token,
      },
    });
  ...
  return (
    <div className="fixed z-50">
      <div>
        ...
        <IoIosAddCircle  className=" text-5xl text-[#1976d2] sm:text-end rounded-2xl" type="primary" onClick={showModal}/>
          ...
          {loading === true ? (
            ...
            <form action="" className="flex flex-col">
              ...
              <input
                ...
                className="outline-none border bg-gray-800 text-white my-2 border-gray-400 py-2 px-4 rounded-md"
                type="text"
              />
              <label className="mb-2" htmlFor="">
                Description
              ...
              />
              <label
                className="mb-2 py-2 px-4 bg-amber-800 w-max text-white rounded-md hover:bg-amber-700"
                htmlFor="file"
              >
                Image/Video
              </label>
              <input
                ...
                id="file"
                hidden
                className="outline-none border my-2 border-gray-400 py-2 px-4 text-white rounded-md"
                type="file"
              />
              <div className="flex gap-5 justify-center ">
                {formDetails.file.map((ele, i) => {
                  return ele.type.split("/")[0] === "image" ? (
                    <img
                      key={i}
                      className="h-32 w-32"
              ...
              <button
                ...
                className="py-2 px-4 bg-sky-950 text-white rounded-md hover:bg-sky-700"
              >
                Submit Post
              </button>
            </form>
